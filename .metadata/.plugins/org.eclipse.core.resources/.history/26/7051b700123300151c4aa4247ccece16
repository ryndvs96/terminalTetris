...
public class testBlocks {
	public static void main(String[] args) {
		...
		System.out.println("\n" + time + " seconds : " + comma(newBlocks) + " new Blocks() : For " + size + " block piece.");
	}
	public class Blocks {
		...
		private char[][] rotate(char[][] block, int times) {
			...
			for (int i = 0; i < size; i++) {
				for (int j = 0; j < size; j++) {
					int newi = size - 1 - j;
					int newj = i;
					copy[newi][newj] = block[i][j] == '\0' || block[i][j] == ' ' ? ' ' : block[i][j];
				}
			}
			if (times > 1) {
		...
		private char[][] blockCopy(char[][] block) {
			...
			for (int i = 0; i < size; i++) {
				for (int j = 0; j < size; j++) {
					blockCopy[i][j] = block[i][j] == '\0' || block[i][j] == ' ' ? ' ' : block[i][j];
				}
			}
			return blockCopy;
		}
			...
			for (int k = 0; k < blocks.size(); k++) {
				...
				for (int i = 0; i < size; i++) {
					for (int j = 0; j < size; j++) {
						...
						if (j + i > size - 1 && block[i][j] != '\0' && block[i][j] == c) {
							return false;
						}
					}
				}
		...
		private void print(char[][] block) {
			for (int i = 0; i < size; i++) {
				for (int j = 0; j < size; j++) {
					char x = block[i][j] == '\0' || block[i][j] == ' ' ? ' ' : block[i][j];
					System.out.print(x + " ");
				}
				System.out.println();
			}
			System.out.println("-----------------" + (blocks.size()) + "--- used " 
